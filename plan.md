...
**Where:** `testlib.h:5667-5716` (`parseExponentialOptValue`), `5758-5767`
(`optValueToLongDouble`)

**What:** traced end to end for the value `"-e-5"`:

1. `pos = 1`, `e = "-5"`, `ne = -5`, `num = s.substr(0, 1) = "-"` (`5685`).
2. `optValueToLongDouble("-")` at `5690` **does not reject it**: `s[0] == '-'`
   sets `sign = -1, pos = 1`, and the digit loop `for (i = pos; i < s.length())`
   never executes, so it returns `-0.0` (`5764-5789`).
3. `num[0] == '-'` strips the sign, leaving `num == ""` (`5692-5694`).
4. The negative-exponent loop runs 5 times; the first iteration finds no `.` and
   evaluates
   `num.insert(num.begin() + int(num.length()) - 1, '.')` =
   `num.insert(num.begin() - 1, '.')` — **an iterator before `begin()`** (`5710`).

The sibling positive-exponent loop at `5696` is safe because it only appends.
...
**Where:** `testlib.h:1477-1485`, `1510-1511`
...
diagnostic. Note that the symmetric `{,5}` *does* fail loudly (`1498-1499`),
...
reject it like `*`, see `1448-1450`), or `__testlib_fail` on it. Failing loudly
...
**Where:** `testlib.h:1614-1639`, falling through to `1598`
...
**What:** the group branch at `1637` requires `firstClose + 1 == s.length()`,
...
`__testlib_fail` at `1628` or `1635` a level or two down.
...
**Where:** `testlib.h:247` (`#define EOFC (255)`), `1661-1663` (`isEof`),
`3410-3417` (`InStream::curChar` / `nextChar`)
...
**Where:** `testlib.h:1987-1992` (`increment`), `2035-2037` (`skipChar`),
`3446-3448` (`InStream::skipChar`), `2039-2047` (`unreadChar`)
...
`skipBlanks`, `3450-3453`), so the library itself is safe — but
`InStream::skipChar()` is public and documented ("Moves stream pointer one
character forward", `2116-2117`) and forwards straight through.
...
**Where:** `testlib.h:3512-3518` (`__testlib_readMany`)
...
**Where:** `testlib.h:5799-5802` (`has_opt`), `5658` (`__testlib_keyToOpts`,
the only place `used` is set), `6352-6356` (`autoEnsureNoUnusedOpts`)
...
**Where:** `testlib.h:5852-5858`, `5908-5911` (`__testlib_indexToArgv`)
...
**Where:** `testlib.h:5579-5589` (`parseOpt`)
...
**Where:** `testlib.h:303-309` (`PC_BASE_EXIT_CODE`), `1715` (`_pc`),
`2998-2999` (`resultExitCode`)
...
below `_partially`, and hits `quit(_fail, "What is the code ??? ")` (`3213`).
...
`_pc(3)`. Only the appes XML (`3224-3226`) carries the real value.
...
**Where:** `testlib.h:2626-2633`, and the same shape at `2636-2637`
(`addVariable`) and `2652-2653` (`adjustConstantBounds`)
...
`testlib.h:965-967`, `977-979`. `to - from + 1` is computed in `long long` and
overflows for wide ranges: `rnd.next(LLONG_MIN, LLONG_MAX)`. `next(int, int)` at
`955` widens first, so the fix pattern is already known and simply not applied
...
`testlib.h:1162-1165`, and the same at `1176`, `1191`. `rnd.wnext(-15e8, 15e8, 1)`
...
`testlib.h:1169-1173`. `next(unsigned, unsigned)` at `960` correctly returns
...
`testlib.h:1270` (`to - from + 1` evaluated in `T`, then widened),
`1318` (`min_part * size` overflows *before* the guard can fire), `1327`
(`sum + size - 1`). Some cases wrap into a state the post-hoc self-checks at
`1343`/`1346` do not catch, so the corruption is silent.
**Reproducer (do not add to the suite).**


### R-08 · `distinct` can spend O(size) divisions then allocate gigabytes · reported
`testlib.h:1274-1290`. The strategy heuristic costs O(size) divisions just to
...
`testlib.h:4342-4358`. When a CR is not followed by LF it is consumed by
...
`testlib.h:3746`. `stringToDouble` checks `__testlib_isNaN` but not
`__testlib_isInfinite`; `stringToStrictDouble` **does** check it (`3827`) — an
...
`testlib.h:1404-1417`, `1423-1439`. `__pattern_greedyMatch` takes the longest
...
anything. Documented at `712-713`, but the consequence is a checker that rejects
...
`testlib.h:4338-4364` grows `result` unbounded, unlike `readWordTo` (`3478-3480`)
which enforces the cap. A 128 MB single-line participant output costs ~400 MB
across geometric growth plus the by-value return at `4367-4368`.

### I-08 · report text is unsanitised outside appes mode · reported
`testlib.h:3236-3237`, `3258` write the message with a bare `%s`; only the XML
branch runs it through `xmlSafeWrite` (`3309-3312`). Participant-controlled ANSI
...
`testlib.h:2424-2425` define `OPEN_BRACKET = char(11)` and
`CLOSE_BRACKET = char(17)`. `isBlanks` (`1672-1674`) accepts neither, so both
are legal inside a token read by `readWord`. A participant emitting
`\x0b…\x11` makes `__testlib_appendMessage` (`3088-3091`) splice
...
`testlib.h:1842-1847` maps `EOF` to `EOFC (255)`, which is exactly what `getc`
...
`testlib.h:1585-1593`. Three problems: the loop runs `code < 255`, so byte
...
`testlib.h:1448-1455`. The guard catches `to == INT_MAX` (so `*` and `+` fail
...
`testlib.h:1639`, `1654`. `children.push_back(pattern(s.substr(pos)))` makes the
tail a nested `pattern`, so depth equals the token count, with an O(n²) `substr`
chain and an O(n²) space-stripping loop (`1605-1607`). A ~100k-character pattern
...
`testlib.h:5582-5588`. `./gen -count -verbose` yields `count = "true"`.
...
(`getOptType("-5")` returns 0, `5525-5535`), but the rule "a value may not start
...
`testlib.h:5951-5953`. `opt<int>("x")` on a missing key FAILs with
...
`testlib.h:5529`, `5532`. Passing a negative value to `isalpha` is UB — only
...
`testlib.h:5747`. `value = T(value * 10 + s[i] - '0')` overflows for signed `T`
*before* the `about` sanity check at `5752` can detect it — the guard is a
...
`testlib.h:4682` is the only call site of `prepareOpts`. In checkers,
...
`testlib.h:2746` and `2787` use `if (c <= 256)` against the encoding at `1888`
...
`setTestCase(1)` (the global at `2957-2974` only latches `zero_based` on the
first call). Fix: `c < 256`.

### V-03 · markup and test-case files truncate at the first NUL · reported
`testlib.h:2768`, `2815` use `fprintf(f, "%s", …)` on `std::string`s built from
...
`testlib.h:2665`, `2681`, `2701`, `2646`. `isVariableNameBoundsAnalyzable`
(`2532-2537`) rejects digits and control characters but not `"` or `\`, so a
...
`testlib.h:2725-2730`, `2768-2769`, `2815-2816` check only `fclose`. On a full
disk `fprintf` fails, `fclose` succeeds, and the validator exits 0 with a
silently truncated log. When the target is `stdout`/`stderr` (`2716`, `2759`,
`2806`) `fclose` is deliberately skipped, so those paths have **no** error
detection at all.

### V-06 · `writeTestCase` writes no file for an empty case · reported
`testlib.h:2803` guards on `!testCaseContent.empty()`, so the caller cannot
distinguish "empty test case" from "case not found" from "validator does not
support extraction". Related: at `2806`, if `--testCaseFileName` is omitted the
...
`testlib.h:4824-4865` uses a chain of independent `if`s (not `else if`) while
...
`registerTestlibCmd` (`4926-4939`) got this right with `else if`. Also `4852`'s
range check and its message disagree by one, and the reader's own check (`1886`)
allows a third range.

### Q-02 · `std::exit` from a destructor invoked by `exit` · traced
`testlib.h:2859-2882` (`~TestlibFinalizeGuard`), `3059` (`halt`). The guard
...
`-DTESTLIB_THROW_EXIT_EXCEPTION_INSTEAD_OF_EXIT` (`3057`) it instead *throws*
...
`testlib.h:4988` sets `argv[0] = NULL`, then `registerTestlibCmd` constructs
`std::vector<std::string> args(1, argv[0])` at `4923` — `std::string(nullptr)`
...
`testlib.h:3231-3232`. `__testlib_preparePoints` (`4505-4520`) is reached only
...
(`4564-4571`) rejects only spaces, accepting `""` and embedded newlines.


### F-02 · `FMT_TO_RESULT`'s reentrancy guard recurses infinitely · traced
`testlib.h:336-347`, `5026-5030`. The guard reports its own violation via
...
clean `_fail`. The counter is also not exception-safe: a `bad_alloc` at `345`
...
`testlib.h:470-473`, `482-487`. `__testlib_isInfinite` is a magnitude test, not
...
`testlib.h:582-589`. The non-integral overload does `ss << t; ss >> s;`, so
...
`testlib.h:1603-1607`. The constructor removes every unescaped space from the
...
`testlib.h:1393-1400`. `__pattern_getChar` returns the raw character after a
...
`testlib.h:3193-3198`. `_dirt` is rewritten to `_pe` before `resultExitCode`, so
...
`testlib.h:3159-3164`. Any non-`_fail` verdict raised on a stream whose
...
`testlib.h:838-841` masks to 48 bits, so `rnd.setSeed(1)` and
`rnd.setSeed(1 + (1LL << 48))` produce byte-identical streams.

### A-06 · `shuffle` hard-codes the global `rnd` · traced
`testlib.h:5046-5050`. There is no `shuffle(first, last, random_t&)`, so a
...
`testlib.h:2534`. `readInt(1, n, "a1")` is completely absent from the bounds-hit
...
`testlib.h:2606-2616`, `2471-2476`. `n`, `~n`, `n~` and `~n~` all collapse to the
...
`testlib.h:2864-2868` checks only `_checker` (must quit) and `_validator` (must
...
**random:** `R-09` `next(double,double)` can return `to` (`1003-1007`), and
`wnext(double,int)` crops inconsistently across the `lim = 25` boundary (`1115`
vs `1124`) · `R-10` `abs(INT_MIN)` and `type + 1` are UB in `wnext` (`1052`,
`1066-1068`), with `wnext(n, INT_MAX)` degrading **silently** to a constant rather
...
`std::iterator_traits` (`1022`) · `R-13` `rnd.any` on a `std::set`/`std::list` is
O(n) per call via `std::advance`.

**pattern:** `P-07` `{-5,3}` is accepted and behaves like `{0,3}` with a skewed
length distribution (`1500-1516`) · `P-08` `rnd.next("[]")` fails with a
`random_t` message rather than a pattern one (`1454`) · `P-09`
`__pattern_greedyMatch` takes its `std::vector<char>` **by value** and is marked
`__attribute__((pure))` while allocating (`1404`) · `P-10`
`__pattern_isCommandChar` walks back over all preceding backslashes on every
call, making construction O(n²) on backslash-heavy patterns (`1386-1388`).
...
`unsigned long long` (`3911` vs `3886`) · `I-12` `maxMessageLength` underflows
when set below the warning-prefix length, making the message *longer* after
truncation (`3145-3149`) · `I-13` a missing input file leaves `reader == NULL`
and `readWordTo` dereferences it, unlike the guarded `eof()` (`3327-3354` vs
`4240`) · `I-14` `maxFileSize` is not enforced by the `FILE*` `init` overload
(`3379-3392`), so the 128 MB guarantee does not hold for interactors ·
`I-15` `int(size_t)` casts print a negative size in the file-too-big message
(`3374`) · `I-16` NUL bytes in a participant token truncate the report at
`.c_str()` · `I-17` `StringInputStreamReader::unreadChar` writes into the source
string (`1812-1813`) · `I-18` the 2 MB buffer and its `isEof` array are never
zeroed (`1995-1998`) · `I-19` `refill()` issues a fresh `fread` on every
`curChar`/`eof` once the file is exhausted (`1954-1980`) · `I-20`
`__testlib_part` builds a full copy of a 32 MB token to produce a 64-character
excerpt, on the error path (`3499-3510`), and `t.substr(s.length() - 31, 31)` at
`3509` should use `t.length()` — provably equal today, wrong by intent.

**opts / validator / verdict:** `O-10` duplicate keys silently last-win
(`5591`) · `O-11` `prepareOpts` fails when `argc <= 0` but prints
"expected argc>=0" (`5629`) · `V-08` `writeTestMarkup`'s error message names
`_testCaseFileName` (`2772`) · `V-09` `ConstantBound` has no constructor
(`2479-2502`) and `std::to_string(double)` gives 6 decimals, so
...
than `== OK_EXIT_CODE` (`2877`), so under `-DCONTESTER` (`OK_EXIT_CODE 0xAC`)
the validator logs are silently never written · `Q-06` `startTest(-3)` creates a
file literally named `-3` (`5100-5104`), and the `"wt"` mode string is an MSVC
...
(`6375-6389`); the C++20 and fallback implementations also differ in truncation
...
`__attribute__((format))` (`6369-6372`, `6398-6401`); every in-tree call site
...
(`5110`) · `F-08` `__testlib_format_buffer` is a **16 MB static array**
(`333`), so every testlib binary carries 16 MB of BSS and is inherently
non-reentrant.
...
- **B-20 · C++03 is broken at link.** `TestlibFinalizeGuard testlibFinalizeGuard;`
  is *defined* at `6358`, inside the `#if __cplusplus > 199711L || defined(_MSC_VER)`
  block that opens at `5317`, while the `extern` declaration (`2891`) and its
  uses in `InStream::quit` (`3127`) and `InStream::readEof` (`4316`) are outside
...
- **S-05 · `opt<long long>` above 2^53.** `testlib.h:5752` uses
...
- **O-12 · `-n=1e2000000000`.** The exponent is bounded only by `int` range, so
  the positive loop at `5696` appends ~2×10^9 characters. A hang/OOM reachable
