...
### R-01 · `rnd.next(0, 1)` repeats every 65536 calls · CRITICAL · measured

**Where:** `testlib.h:765-780` (`nextBits`), `854-866` (`next(long long)`),
`888-890` (`next(int, int)`)

**What:** `nextBits(63)` builds its result as

```cpp
long long left  = (nextBits(31) << 32);   // occupies result bits 32..62
long long right =  nextBits(lowerBitCount);
return left ^ right;                      // right occupies bits 0..31
```

The two operands sit in **disjoint bit ranges**, so the XOR is a plain
concatenation — no mixing happens at all. Result bit *j* (for j ≤ 31) is
therefore state bit *j+16* of the 48-bit LCG, i.e. the low-order bits, which
are exactly the bits an LCG is worst at. `next(long long n)` then takes
`bits % n`, reading those weak low bits directly.

The trap is that **`next(int, int)` widens to `long long` before dispatching**,
so the ordinary-looking `rnd.next(0, 1)` takes the weak path, while
`rnd.next(2)` — the `int` overload, which uses the *top* 31 bits — does not.

**Measured periods** (probe over 3× the expected period, two seeds; the period
is seed-independent):

| Call | version 1 / unset | version 0 |
| --- | --- | --- |
| `rnd.next(0, 1)` | **65536** | 131072 |
| `rnd.next(2LL)` | **65536** | — |
| `rnd.next(0, 3)` | 131072 | — |
| `rnd.next(0, 7)` | 262144 | — |
| `rnd.next(0, 15)` | 524288 | — |
| `rnd.next(0, 255) & 1` | 65536 | — |
| `rnd.next(2)` *(int path)* | none in 196608 draws | — |

General rule: `next(2^k)` on the `long long` path has period **2^(15+k)** calls
under version 1 and 2^(16+k) under version 0.

**Trigger:**
```cpp
registerGen(argc, argv, 1);
for (int i = 0; i < 200000; i++) printf("%d", rnd.next(0, 1));  // repeats after 65536
```

**Impact:** a generator emitting a random bitstring, a 0/1 matrix, a small
alphabet, or coin-flip branch decisions produces a visibly periodic file. Two
tests drawing more than 65536 flips share long identical runs. For
`rnd.next(0, 255)` the values are *consecutive low LCG bits* — structured, not
merely repeating. This silently defeats the entire purpose of the library's
randomness, in its most-used call.

**Pinned by:** `tests/test-004_use-test.h/tests/test-rnd-quality.cpp`

**Fix:** under `random_t::version == 2`, build the 63-bit value from top-bit
draws only, e.g. `(nextBits(32) << 31) | nextBits(31)`, so no result bit comes
from a state bit below 16. Gate it on the new version; leave 0 and 1 untouched.
Add a `test-003`-style ref test for version 2. See also **R-02**, **R-07**.
...
**Where:** `testlib.h:5600-5649` (`parseExponentialOptValue`), `5691-5700`
(`optValueToLongDouble`)

**What:** traced end to end for the value `"-e-5"`:

1. `pos = 1`, `e = "-5"`, `ne = -5`, `num = s.substr(0, 1) = "-"` (`5618`).
2. `optValueToLongDouble("-")` at `5623` **does not reject it**: `s[0] == '-'`
   sets `sign = -1, pos = 1`, and the digit loop `for (i = pos; i < s.length())`
   never executes, so it returns `-0.0` (`5697-5722`).
3. `num[0] == '-'` strips the sign, leaving `num == ""` (`5625-5627`).
4. The negative-exponent loop runs 5 times; the first iteration finds no `.` and
   evaluates
   `num.insert(num.begin() + int(num.length()) - 1, '.')` =
   `num.insert(num.begin() - 1, '.')` — **an iterator before `begin()`** (`5643`).

The sibling positive-exponent loop at `5629` is safe because it only appends.
...
**Fix:** version 0 must stay byte-identical, so this is **documentation only**:
state in `docs/usage-guide.md` that `registerGen(..., 0)` is for byte-compatible
rebuilds of old generators and must not be used for new ones.

---

### P-01 · `{n,}` silently means `{n,n}` · HIGH · measured

**Where:** `testlib.h:1410-1418`, `1443-1444`
...
diagnostic. Note that the symmetric `{,5}` *does* fail loudly (`1431-1432`),
...
reject it like `*`, see `1381-1383`), or `__testlib_fail` on it. Failing loudly
...
**Where:** `testlib.h:1547-1572`, falling through to `1531`
...
**What:** the group branch at `1570` requires `firstClose + 1 == s.length()`,
...
`__testlib_fail` at `1561` or `1568` a level or two down.
...
**Where:** `testlib.h:238` (`#define EOFC (255)`), `1594-1596` (`isEof`),
`3343-3350` (`InStream::curChar` / `nextChar`)
...
**Where:** `testlib.h:1920-1925` (`increment`), `1968-1970` (`skipChar`),
`3379-3381` (`InStream::skipChar`), `1972-1980` (`unreadChar`)
...
`skipBlanks`, `3383-3386`), so the library itself is safe — but
`InStream::skipChar()` is public and documented ("Moves stream pointer one
character forward", `2049-2050`) and forwards straight through.
...
**Where:** `testlib.h:3445-3451` (`__testlib_readMany`)
...
**Where:** `testlib.h:5732-5735` (`has_opt`), `5591` (`__testlib_keyToOpts`,
the only place `used` is set), `6285-6289` (`autoEnsureNoUnusedOpts`)
...
**Where:** `testlib.h:5785-5791`, `5841-5844` (`__testlib_indexToArgv`)
...
**Where:** `testlib.h:5512-5522` (`parseOpt`)
...
**Where:** `testlib.h:294-300` (`PC_BASE_EXIT_CODE`), `1648` (`_pc`),
`2931-2932` (`resultExitCode`)
...
below `_partially`, and hits `quit(_fail, "What is the code ??? ")` (`3146`).
...
`_pc(3)`. Only the appes XML (`3157-3159`) carries the real value.
...
**Where:** `testlib.h:2559-2566`, and the same shape at `2569-2570`
(`addVariable`) and `2585-2586` (`adjustConstantBounds`)
...
`testlib.h:898-900`, `910-912`. `to - from + 1` is computed in `long long` and
overflows for wide ranges: `rnd.next(LLONG_MIN, LLONG_MAX)`. `next(int, int)` at
`888` widens first, so the fix pattern is already known and simply not applied
...
`testlib.h:1095-1098`, and the same at `1109`, `1124`. `rnd.wnext(-15e8, 15e8, 1)`
...
`testlib.h:1102-1106`. `next(unsigned, unsigned)` at `893` correctly returns
...
`testlib.h:1203` (`to - from + 1` evaluated in `T`, then widened),
`1251` (`min_part * size` overflows *before* the guard can fire), `1260`
(`sum + size - 1`). Some cases wrap into a state the post-hoc self-checks at
`1276`/`1279` do not catch, so the corruption is silent.
**Reproducer (do not add to the suite).**

### R-07 · `setSeed(argc, argv)` is not reproducible across architectures · traced
`testlib.h:791-803`. `(unsigned int)(argv[i][j])` converts a **`char`**: byte
...
produce correlated coin-flip streams. Dead `random_t p;` at `792`.
...
`testlib.h:1207-1223`. The strategy heuristic costs O(size) divisions just to
...
`testlib.h:4275-4291`. When a CR is not followed by LF it is consumed by
...
`testlib.h:3679`. `stringToDouble` checks `__testlib_isNaN` but not
`__testlib_isInfinite`; `stringToStrictDouble` **does** check it (`3760`) — an
...
`testlib.h:1337-1350`, `1356-1372`. `__pattern_greedyMatch` takes the longest
...
anything. Documented at `703-704`, but the consequence is a checker that rejects
...
`testlib.h:4271-4297` grows `result` unbounded, unlike `readWordTo` (`3411-3413`)
which enforces the cap. A 128 MB single-line participant output costs ~400 MB
across geometric growth plus the by-value return at `4300-4301`.

### I-08 · report text is unsanitised outside appes mode · reported
`testlib.h:3169-3170`, `3191` write the message with a bare `%s`; only the XML
branch runs it through `xmlSafeWrite` (`3242-3245`). Participant-controlled ANSI
...
`testlib.h:2357-2358` define `OPEN_BRACKET = char(11)` and
`CLOSE_BRACKET = char(17)`. `isBlanks` (`1605-1607`) accepts neither, so both
are legal inside a token read by `readWord`. A participant emitting
`\x0b…\x11` makes `__testlib_appendMessage` (`3021-3024`) splice
...
`testlib.h:1775-1780` maps `EOF` to `EOFC (255)`, which is exactly what `getc`
...
`testlib.h:1518-1526`. Three problems: the loop runs `code < 255`, so byte
...
`testlib.h:1381-1388`. The guard catches `to == INT_MAX` (so `*` and `+` fail
...
`testlib.h:1572`, `1587`. `children.push_back(pattern(s.substr(pos)))` makes the
tail a nested `pattern`, so depth equals the token count, with an O(n²) `substr`
chain and an O(n²) space-stripping loop (`1538-1540`). A ~100k-character pattern
...
`testlib.h:5515-5521`. `./gen -count -verbose` yields `count = "true"`.
...
(`getOptType("-5")` returns 0, `5458-5468`), but the rule "a value may not start
...
`testlib.h:5884-5886`. `opt<int>("x")` on a missing key FAILs with
...
`testlib.h:5462`, `5465`. Passing a negative value to `isalpha` is UB — only
...
`testlib.h:5680`. `value = T(value * 10 + s[i] - '0')` overflows for signed `T`
*before* the `about` sanity check at `5685` can detect it — the guard is a
...
`testlib.h:4615` is the only call site of `prepareOpts`. In checkers,
...
`testlib.h:2679` and `2720` use `if (c <= 256)` against the encoding at `1821`
...
`setTestCase(1)` (the global at `2890-2907` only latches `zero_based` on the
first call). Fix: `c < 256`.

### V-03 · markup and test-case files truncate at the first NUL · reported
`testlib.h:2701`, `2748` use `fprintf(f, "%s", …)` on `std::string`s built from
...
`testlib.h:2598`, `2614`, `2634`, `2579`. `isVariableNameBoundsAnalyzable`
(`2465-2470`) rejects digits and control characters but not `"` or `\`, so a
...
`testlib.h:2658-2663`, `2701-2702`, `2748-2749` check only `fclose`. On a full
disk `fprintf` fails, `fclose` succeeds, and the validator exits 0 with a
silently truncated log. When the target is `stdout`/`stderr` (`2649`, `2692`,
`2739`) `fclose` is deliberately skipped, so those paths have **no** error
detection at all.

### V-06 · `writeTestCase` writes no file for an empty case · reported
`testlib.h:2736` guards on `!testCaseContent.empty()`, so the caller cannot
distinguish "empty test case" from "case not found" from "validator does not
support extraction". Related: at `2739`, if `--testCaseFileName` is omitted the
...
`testlib.h:4757-4798` uses a chain of independent `if`s (not `else if`) while
...
`registerTestlibCmd` (`4859-4872`) got this right with `else if`. Also `4785`'s
range check and its message disagree by one, and the reader's own check (`1819`)
allows a third range.

### Q-02 · `std::exit` from a destructor invoked by `exit` · traced
`testlib.h:2792-2815` (`~TestlibFinalizeGuard`), `2992` (`halt`). The guard
...
`-DTESTLIB_THROW_EXIT_EXCEPTION_INSTEAD_OF_EXIT` (`2990`) it instead *throws*
...
`testlib.h:4921` sets `argv[0] = NULL`, then `registerTestlibCmd` constructs
`std::vector<std::string> args(1, argv[0])` at `4856` — `std::string(nullptr)`
...
`testlib.h:3164-3165`. `__testlib_preparePoints` (`4438-4453`) is reached only
...
(`4497-4504`) rejects only spaces, accepting `""` and embedded newlines.


### F-02 · `FMT_TO_RESULT`'s reentrancy guard recurses infinitely · traced
`testlib.h:327-338`, `4959-4963`. The guard reports its own violation via
...
clean `_fail`. The counter is also not exception-safe: a `bad_alloc` at `336`
...
`testlib.h:461-464`, `473-478`. `__testlib_isInfinite` is a magnitude test, not
...
`testlib.h:573-580`. The non-integral overload does `ss << t; ss >> s;`, so
...
`testlib.h:1536-1540`. The constructor removes every unescaped space from the
...
`testlib.h:1326-1333`. `__pattern_getChar` returns the raw character after a
...
`testlib.h:3126-3131`. `_dirt` is rewritten to `_pe` before `resultExitCode`, so
...
`testlib.h:3092-3097`. Any non-`_fail` verdict raised on a stream whose
...
`testlib.h:806-809` masks to 48 bits, so `rnd.setSeed(1)` and
`rnd.setSeed(1 + (1LL << 48))` produce byte-identical streams.

### A-06 · `shuffle` hard-codes the global `rnd` · traced
`testlib.h:4979-4983`. There is no `shuffle(first, last, random_t&)`, so a
...
`testlib.h:2467`. `readInt(1, n, "a1")` is completely absent from the bounds-hit
...
`testlib.h:2539-2549`, `2404-2409`. `n`, `~n`, `n~` and `~n~` all collapse to the
...
`testlib.h:2797-2801` checks only `_checker` (must quit) and `_validator` (must
...
**random:** `R-09` `next(double,double)` can return `to` (`936-940`), and
`wnext(double,int)` crops inconsistently across the `lim = 25` boundary (`1048`
vs `1057`) · `R-10` `abs(INT_MIN)` and `type + 1` are UB in `wnext` (`985`,
`999-1001`), with `wnext(n, INT_MAX)` degrading **silently** to a constant rather
...
`std::iterator_traits` (`955`) · `R-13` `rnd.any` on a `std::set`/`std::list` is
O(n) per call via `std::advance`.

**pattern:** `P-07` `{-5,3}` is accepted and behaves like `{0,3}` with a skewed
length distribution (`1433-1449`) · `P-08` `rnd.next("[]")` fails with a
`random_t` message rather than a pattern one (`1387`) · `P-09`
`__pattern_greedyMatch` takes its `std::vector<char>` **by value** and is marked
`__attribute__((pure))` while allocating (`1337`) · `P-10`
`__pattern_isCommandChar` walks back over all preceding backslashes on every
call, making construction O(n²) on backslash-heavy patterns (`1319-1321`).
...
`unsigned long long` (`3844` vs `3819`) · `I-12` `maxMessageLength` underflows
when set below the warning-prefix length, making the message *longer* after
truncation (`3078-3082`) · `I-13` a missing input file leaves `reader == NULL`
and `readWordTo` dereferences it, unlike the guarded `eof()` (`3260-3287` vs
`4173`) · `I-14` `maxFileSize` is not enforced by the `FILE*` `init` overload
(`3312-3325`), so the 128 MB guarantee does not hold for interactors ·
`I-15` `int(size_t)` casts print a negative size in the file-too-big message
(`3307`) · `I-16` NUL bytes in a participant token truncate the report at
`.c_str()` · `I-17` `StringInputStreamReader::unreadChar` writes into the source
string (`1745-1746`) · `I-18` the 2 MB buffer and its `isEof` array are never
zeroed (`1928-1931`) · `I-19` `refill()` issues a fresh `fread` on every
`curChar`/`eof` once the file is exhausted (`1887-1913`) · `I-20`
`__testlib_part` builds a full copy of a 32 MB token to produce a 64-character
excerpt, on the error path (`3432-3443`), and `t.substr(s.length() - 31, 31)` at
`3442` should use `t.length()` — provably equal today, wrong by intent.

**opts / validator / verdict:** `O-10` duplicate keys silently last-win
(`5524`) · `O-11` `prepareOpts` fails when `argc <= 0` but prints
"expected argc>=0" (`5562`) · `V-08` `writeTestMarkup`'s error message names
`_testCaseFileName` (`2705`) · `V-09` `ConstantBound` has no constructor
(`2412-2435`) and `std::to_string(double)` gives 6 decimals, so
...
than `== OK_EXIT_CODE` (`2810`), so under `-DCONTESTER` (`OK_EXIT_CODE 0xAC`)
the validator logs are silently never written · `Q-06` `startTest(-3)` creates a
file literally named `-3` (`5033-5037`), and the `"wt"` mode string is an MSVC
...
(`6308-6322`); the C++20 and fallback implementations also differ in truncation
...
`__attribute__((format))` (`6302-6305`, `6331-6334`); every in-tree call site
...
(`5043`) · `F-08` `__testlib_format_buffer` is a **16 MB static array**
(`324`), so every testlib binary carries 16 MB of BSS and is inherently
non-reentrant.
...
- **B-20 · C++03 is broken at link.** `TestlibFinalizeGuard testlibFinalizeGuard;`
  is *defined* at `6291`, inside the `#if __cplusplus > 199711L || defined(_MSC_VER)`
  block that opens at `5250`, while the `extern` declaration (`2824`) and its
  uses in `InStream::quit` (`3060`) and `InStream::readEof` (`4249`) are outside
...
- **S-05 · `opt<long long>` above 2^53.** `testlib.h:5685` uses
...
- **O-12 · `-n=1e2000000000`.** The exponent is bounded only by `int` range, so
  the positive loop at `5629` appends ~2×10^9 characters. A hang/OOM reachable
